		...
		case MAIN:
			{
				...
				for (int i = 0;i<objects.size();i++) {
					if (leftMouseReleased&&objects[i]->clickedIn(mouseX,mouseY)) {
						if (!shiftClicked) {
				...
				if (runningVideo) {
					...
					for (int i = 0;i<selectedObjects.size();i++) {
						...
						if (leftMouseReleased&&pointInBounds(mouseX, mouseY, totoff, totoff+w2, controlBarY+5+h, controlBarY+5+h+h2)) {
						...
						if (leftMouseReleased&&pointInBounds(mouseX, mouseY, totoff, totoff+w3, controlBarY+5+h+h2, controlBarY+5+h+h2+h3)) {
						...
						if (leftMouseReleased&&pointInBounds(mouseX, mouseY, totoff, totoff+w4, controlBarY+5+h+h2+h3, controlBarY+5+h+h2+h3+h4)) {
					...
					if (leftMouseReleased&&pointInBounds(mouseX, mouseY, SCREEN_WIDTH-200, SCREEN_WIDTH-200+rax,controlBarY+5,controlBarY+5+ray)) {
					...
					if (leftMouseReleased&&pointInBounds(mouseX, mouseY, SCREEN_WIDTH-200-5-rsx, SCREEN_WIDTH-200-5-rsx+rsx,controlBarY+5,controlBarY+5+rsy)) {
						for (DisplayObject* d : selectedObjects) {
							d->run();
						}
						runningVideo = true;
					...
					if (leftMouseReleased&&pointInBounds(mouseX, mouseY, SCREEN_WIDTH-200, SCREEN_WIDTH-200+savex,controlBarY+5+rsy+5,controlBarY+5+savey+rsy+5)) {
						//save();
					...
					if (leftMouseReleased&&pointInBounds(mouseX, mouseY, SCREEN_WIDTH-200-5-savex, SCREEN_WIDTH-200-5-savex+loadx,controlBarY+5+rsy+5,controlBarY+5+savey+rsy+5)) {
