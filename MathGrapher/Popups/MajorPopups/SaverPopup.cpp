...
Uint8 SaverPopup::handle(double mouseX,double mouseY) {
	...
	if (clickedEdit) {toReturn = 0x01;clicked.unclick();}
	TTF_SizeUTF8((*fontgrab)(16), "Save", &offx, &offy);
	drawTextWithBackground("Save", 16, px+sx-offx, py, 0xff000000, 0xff9fc9f2, 0xff000000);
	if (clicked.status()&&pointInBounds(mouseX, mouseY, px+sx-offx, px+sx, py, py+offy)) {
	...
	if (clicked.status()&&pointInBounds(mouseX, mouseY, px+sx-offx, px+sx, py+sy-offy, py+sy)) {
		toReturn = 0x02;
	}
