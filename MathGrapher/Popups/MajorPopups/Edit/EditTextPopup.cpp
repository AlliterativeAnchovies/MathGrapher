...
Uint8 EditTextPopup::handle(double mouseX,double mouseY) {
	Uint8 toReturn = 0x00;
	...
	bool clickedInterpol = drawInterpolationSidebar(px+5*sx/8,py,clicked,mouseX,mouseY,getConcernationSD<RawText*>());
	if (clickedInterpol) {
		clicked = false;
		toReturn = 0x01;
	}
